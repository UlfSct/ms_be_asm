#!/usr/bin/env python3
"""
Скрипт для сбора исходного кода Vue проекта в DOCX файл.
Исключает служебные директории (node_modules, dist, .git и т.д.)
"""
...
# Директории и файлы, которые нужно исключить
EXCLUDED_DIRS = {
    'node_modules',
    'dist',
    '.git',
...
EXCLUDED_FILES = {
    ...
    '.env.production',
    '.env.test',
    # Дополнительные файлы для исключения
    '.editorconfig',
    '.gitignore',
    '.prettierrc.json',
    'eslint.config.js',
...
# Файлы конфигурации, которые стоит включить (проверяется полное имя)
IMPORTANT_CONFIGS = {
    'tsconfig.json',
    'vite.config.js',
    'vite.config.ts',
...
def create_document(root_path, output_path=None):
    ...
    if output_path is None:
        project_name = root.name or 'vue-project'
...
def main():
    parser = argparse.ArgumentParser(
        description='Сбор исходного кода Vue проекта в DOCX файл с двумя колонками'
    )
    parser.add_argument(
        'path',
        type=str,
        help='Абсолютный путь к корню Vue проекта'
    )
    parser.add_argument(
        '-o', '--output',
        type=str,
...
if __name__ == '__main__':
    main()
    #  python.exe .\collect_front_code_to_docx.py "C:\Users\anto8\OneDrive\рабочий стол\диплом\fe_asm_3d"
